...
namespace MyRabbitMqService.BL.Services
{
    public class MyRabbitMqConsumer : IHostedService, IDisposable
    {
        ...
        public MyRabbitMqConsumer(IUserDataFlow userDataFlow)
        {
            ...
            _channel.ExchangeDeclare("domainuser", ExchangeType.Fanout, durable: false);
            ...
            consumer.Received += (sender, ea) =>
            {
                ...
                Console.WriteLine($"id {user.Id} - DomainName {user.DomainName} - Last Update Datetime {user.LUPD_Datetime}");
                _userDataFlow.SendDomainUSR(ea.Body.ToArray());
                UserCache.Cache.Add(user);
            };


            _channel.BasicConsume("domainuser", true, consumer);
